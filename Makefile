...
clean:
	rm -rf .venv .cache/make game.egg-info .pytest_cache tests/.pytest_cache dist requirements-dev-*.txt
...
${VENV_BIN}activate: Makefile .devcontainer/devcontainer.dockerfile
	${PYTHON} -m venv --clear --prompt='game' .venv
.PHONY: venv
...
.cache/make/test: .cache/make/format ${PACKAGE_SRC} ${PACKAGE_DATA} ${TESTS_SRC} ${TESTS_DATA}
	${VENV_BIN}pytest --cov=game --cov-report=term-missing tests
	${TOUCH}
.PHONY: test
